...
void Adafruit_ST7735::initR(uint8_t options) {
  ...
  if (options == INITR_GREENTAB) {
    ...
    _colstart = 26;
    _rowstart = 1;
    invertDisplay(1); // Comment this line if your display isn't inverting colors
  } else {
  ...
  if (options == INITR_HALLOWING) {
    ...
    setRotation(2);
  } else if (options == INITR_MINI160x80_MOD) {  // Remove this condition if your display isn't upside-down
    setRotation(3);
  } else {
    tabcolor = options;
    setRotation(0);
